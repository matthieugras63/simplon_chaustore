...
if (!empty($_POST['update']) && !empty($_POST['updateType1']) && !empty($_POST['updateType2'])) {
  if ($_POST['updateType1'] === $_POST['updateType2']) {
    ...
    $checkIfExist = "SELECT name FROM brand WHERE name = '$nameUpdated';";
    $req = mysqli_query($conn, $checkIfExist);
    $donnees = mysqli_fetch_row($req);
    if ($donnees[0] !== NULL) {
      $msgErr .= "<br/> la marque " . $_POST['updateType1']." est déjà référencée ";
    } else {
      $update = "UPDATE brand SET name = '$nameUpdated' WHERE name = '$toUpdate';";
...
if(isset($_POST["submit"]) && $msgErr === '') {
  ...
  if(empty($_POST["update"]) || empty($_POST['updateType1']) || empty($_POST['updateType2'])) {
    ...
    $msgSucc .="<br/> La marque " . $_POST['update']." a bien été modifiée avec succès en " . $_POST['updateType1']." " ;
  }
}
?>
...
<body>
  <main>
    <h2>Modifier une marque de chaussure</h2>
    <div>
      ...
      <form action="update_brand.php" method="POST">
        <p>
          <label for="update">Marque :</label>
          <select name="update" id="brand">
